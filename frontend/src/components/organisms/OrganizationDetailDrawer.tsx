...
export function OrganizationDetailDrawer({ organizationId, onClose, onUpdate }: OrganizationDetailDrawerProps) {
  ...
  return <Dialog eyebrow="Portefeuille relationnel" title={data?.name || 'Établissement'} onClose={onClose}><div className={classes.content}>
    ...
    {!loading && data && <><div className={classes.meta}><OrganizationTypeBadge type={data.type} size="sm" /><div className={classes.links}>{data.linkedin_url && <a className={classes.link} href={data.linkedin_url} target="_blank" rel="noreferrer">LinkedIn</a>}{data.website && <a className={classes.link} href={data.website} target="_blank" rel="noreferrer">Site web</a>}</div></div><Tabs label="Détails de l’établissement" value={tab} items={[["overview", "Aperçu"], ["applications", "Candidatures"], ["contacts", "Contacts"]] as const} onChange={setTab} />
      {tab === 'overview' && <div className={classes.panel}><ProbityBadge score={data.metrics.probity_score} level={data.metrics.probity_level} size="md" /><section className={classes.section}><p className={classes.label}>À propos</p><p className={classes.muted}>{data.notes || 'Aucune note.'}</p><div className={classes.grid}><div><p className={classes.label}>Localisation</p><p className={classes.value}>{data.city || 'Non spécifiée'}</p></div><div><p className={classes.label}>Créé le</p><p className={classes.value}>{new Date(data.created_at).toLocaleDateString()}</p></div></div></section></div>}
      {tab === 'applications' && <div className={classes.panel}><div className={classes.list}>{data.applications?.length ? data.applications.map((application) => <div className={classes.row} key={application.id}><div><p className={classes.value}>{application.title}</p><p className={classes.muted}>{new Date(application.applied_at).toLocaleDateString()}</p></div><StatusBadge status={application.status} /></div>) : <p className={classes.muted}>Aucune candidature pour cet établissement.</p>}</div></div>}
  ...
  </div>{editing && data && <OrganizationEditModal organization={data} onClose={() => setEditing(false)} onSaved={async () => { setEditing(false); await loadData(); onUpdate(); }} />}</Dialog>;
}
