...
export function ApplicationDetailDrawer({
  ...
  useEffect(() => {
    ...
    applicationService
      ...
      .catch(() => {
        if (active)
          setError('Cette candidature est temporairement indisponible.');
      })
      .finally(() => {
  ...
  return (
    <Dialog
      eyebrow="Fiche candidature"
      title={data?.application.title || 'Candidature'}
      onClose={onClose}
    >
      <div className={classes.content}>
        {loading && (
          <LoadingStatus>Chargement de la candidature…</LoadingStatus>
        )}
        {error && (
        ...
        {!loading && data && (
          <>
            ...
            <Tabs
              label="Détails de la candidature"
              value={tab}
              items={
                [
                  ['overview', 'Aperçu'],
                ...
                <div className={classes.panel}>
                  {data.organization && (
                    <ProbityBadge
                      ...
                      level={
                        data.organization.metrics?.probity_level ||
                        'insuffisant'
                      }
                      size="md"
                    />
                  ...
                  <section className={classes.section}>
                    <p className={classes.label}>Informations</p>
                    <div className={classes.grid}>
                      <div>
                        <p className={classes.label}>Candidaté le</p>
                      ...
                      <div>
                        <p className={classes.label}>Canal</p>
                        <p className={classes.value}>
                          {data.application.channel || 'Non renseigné'}
                        </p>
                      </div>
                    </div>
                  </section>
                    ...
                    <section className={classes.section}>
                      ...
                      <div className={classes.list}>
                        {data.contacts.map((contact: Contact) => (
                          <div className={classes.row} key={contact.id}>
                    ...
                    {data.events?.length ? (
                      data.events.map((event, index) => (
                        <article
                          ...
                          key={event.id || index}
                        >
                          <p className={classes.value}>{event.type}</p>
                          <p className={classes.muted}>
            ...
            <div className={classes.actions}>
              ...
              <Button
                variant="secondary"
                onClick={() => navigate(`/app/candidatures/${appId}`)}
              >
                Ouvrir la fiche
              </Button>
            </div>
        ...
        {!loading && !data && !error && (
          <p className={classes.muted}>Candidature introuvable.</p>
        )}
      </div>
      {showEditModal && data && (
        <ApplicationEditModal
          ...
          onSaved={async (payload) => {
            ...
            setData(await applicationService.getApplication(appId));
